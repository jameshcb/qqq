...
用 [assets/handoff-prompt-template.md](assets/handoff-prompt-template.md)。**自包含**:① 读什么(项目交接文件 + 相关决策 + 本会话关键产出文件路径)② 当前状态一句话 ③ 下一步选项(让下个会话跟用户确认)。缺"读什么"或"下一步"= 不合格。写进 **`/tmp/qqq-handoff-<项目名>.txt`**(`<项目名>` = 项目根目录名,如 `tops`/`yeehang2026`)——按项目命名,避免两窗两项目同时切窗时互相覆盖。
...
```bash
HANDOFF=/tmp/qqq-handoff-$(basename "$PWD").txt   # 按项目命名,两窗两项目不串
...
```
> ⚠️ Step 4 写、Step 6 读必须是**同一个** `/tmp/qqq-handoff-<项目名>.txt`;bash 调用间环境变量不保留,文件名靠 `$(basename "$PWD")` 推导,别指望跨步骤传变量。
