...
- **管行数也管单段长度**:单段 **≤ 800 字符**(约 6-8 行),超了拆成要点 / 下沉细节到 docs/sessions —— ≤100 行总闸挡不住「5000+ 字符塞进一段」的胖单段,逐段也要自检
...
- **软上限 ≤1KB / ≤25 行**(类比 HANDOFF ≤100 行);超了 = 在抄已有内容,删到只剩指针
...
if [ ${#ENC} -le 1900 ]; then
  ...
  PROMPT=$(node -e 'process.stdout.write(encodeURIComponent(process.argv[1]))' "接班:先读 $HANDOFF 全文(读什么 / 当前状态 / 下一步都在里面),照它开场,再跟我确认走哪条。")
fi
...
- HANDOFF 单段塞成 5000+ 字符的巨段(行数没超但段落胖)→ 违 Step 3:单段 ≤ 800 字符,拆要点或下沉到 docs/sessions
