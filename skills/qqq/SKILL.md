...
- **别人照着做的操作口径 → 项目的帮助 / FAQ 内容**（项目声明的位置，如 TOPS = `docs/training/`）。判据 = 本窗有没有改动“别人照旧做就会做错”的东西：新界面、换了填法、改了术语、加了闸。**必须显式写“有”或“无”**，同下一条的姿势，不许跳过不提。
  - 写之前先答两问：① **读者是谁、这个载体有没有权限门** —— 喂 AI 答疑的语料通常整份进 prompt 且零角色过滤，写进去等于全员可读；成本、供应商、底价这类要另走带权限门的载体。② **写的是常青口径还是会过期的快照** —— 条数、金额、“目前有 N 个”属于快照，进 sessions 不进帮助；帮助只留“什么情况怎么做”。
  - 帮助内容有构建期生成物（打包进 bundle 的语料/文档常量）时，改完要重跑生成器并提交生成物，否则同步测试会红或线上仍是旧内容。
...
- 归档、HANDOFF、开场词各写到哪里。
- **帮助 / FAQ 内容本窗有没有新增或更新**（§3 路由那条的结论）：有就说写到哪一篇、给谁看；没有就说“无”。让用户当场看出知识库有没有跟上，而不是几周后发现界面早变了、帮助还停在旧写法。
